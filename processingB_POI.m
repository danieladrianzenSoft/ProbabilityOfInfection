...
makePlot = 0;
paramMeans = readtable('jmp_doe.xlsx');
POI_VEC = zeros(height(paramMeans),1);

for mean = 1:height(paramMeans)
    B_POI = readtable(sprintf('resultsPOI_%d.xlsx',mean));
    
    numInfected = 0;
    for i = 1:size(B_POI,1)
        if B_POI(i,:).isInfected == 1
            numInfected = numInfected + 1;
        end
    end
    
    %numInfected = sum(B_POI.isInfected);
