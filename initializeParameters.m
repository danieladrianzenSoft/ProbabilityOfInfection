...
function D = initializeDrug(params)
    D = Drug;
    D.initialConcentration = params.C0;
    D.effectiveSurfaceArea = 34.8; %cm2
    D.diffCoeffResin = 1e-9; %cm2/s
    D.thicknessResin = 0.38; %cm
    D.diffCoeffLumen = 6*10^(-6);
    D.diffCoeffEpithelium = 7*10^(-8);
    D.diffCoeffStroma = 4*10^(-7);
    D.partitionResinLumen = 1;
    D.partitionLumenEpthelium = 0.75;
    D.partitionEpitheliumStroma = 1;
    D.dilutionFractionLumen = params.Vg / (params.Vf + params.Vg + params.Vs);
    %D.lossDilutionInLumen = 1.22/3600; % FROM JING'S 2ND PAPER, USE THIS ONE
    D.lossDilutionInLumen = 2.44/3600; % 2x value for testing purposes.
    %D.lossDilutionInLumen = 0.122/3600; % 1/10 of value for testing purposes
    D.lossStromaToBlood = 0.119/3600; %FROM JING'S 2ND PAPER, USE THIS ONE
    %D.lossStromaToBlood = 1.19/3600; % x10 of value for testing purposes
    D.lossClearanceInBlood = 1.41/3600;
    D.volumeOfDistribution = 75*1000;
    D.volumeGel = params.Vg;
...
function V = initializeVirus(params)
    ...
    %V.lossDilutionInLumen = 1.22/3600; %k_D % FROM JING'S 2ND PAPER, USE THIS ONE
    %V.lossDilutionInLumen = 0.122/3600; % 1/10 of value for testing purposes
    V.lossDilutionInLumen = 2.44/3600; % 2x of value for testing purposes
    ...
    V.dilutionFractionLumen = params.Vs / (params.Vf + params.Vg + params.Vs);
    V = V.setSymbols();
end
...
function Geometry = initializeGeometry(params)
    Geometry = Vagina;
    Geometry.surfaceArea = params.SA;
    %Geometry.thicknessLumen = 0.04; %h_G cm
    Geometry.thicknessLumen = 1/2 * ((params.Vf + params.Vs + params.Vg) / Geometry.surfaceArea); %h_G cm
    ...
    Geometry.volumeVaginalFluid = params.Vf;
