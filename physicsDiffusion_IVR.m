function DVDT = physicsDiffusion_IVR(t,sol,IC,Geometry,Mesh,V,T,I,D,U,includeOutputs)
    ...
    D_RIndices = Mesh.numX + 1 : Mesh.numX + Mesh.numR;
    D_intf_RL = Mesh.numX + Mesh.numR;
    D_LIndices = Mesh.numX + Mesh.numR + 1 : Mesh.numX + Mesh.numR + Mesh.numL;
    D_intf_LE = Mesh.numX + Mesh.numR + Mesh.numL;
    D_EIndices = Mesh.numX + Mesh.numR + Mesh.numL + 1 : Mesh.numX + Mesh.numR + Mesh.numL + Mesh.numE;
    D_intf_ES = Mesh.numX + Mesh.numR + Mesh.numL + Mesh.numE;
    D_SIndices = Mesh.numX + Mesh.numR + Mesh.numL + Mesh.numE + 1 : Mesh.numX + Mesh.numR + Mesh.numX;
    DP_EIndices = Mesh.numR + 2 * Mesh.numX + 1 : Mesh.numR + 2 * Mesh.numX + Mesh.numE;
    DP_SIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numE + 1 : Mesh.numR + 2 * Mesh.numX + Mesh.numT;
    T_SIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 1 : Mesh.numR + 2 * Mesh.numX + Mesh.numT + Mesh.numS;
    I_SIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numT + Mesh.numS + 1 : Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS;
    V_BIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 1;
    D_BIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 2;
    DP_BIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 3;
    T_BIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 4;
    I_BIndices = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 5;

    if (includeOutputs == 1)
        VirusCellStroma = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 6;
        CellCellStroma = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 7;
        VirusCellBlood = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 8;
        CellCellBlood = Mesh.numR + 2 * Mesh.numX + Mesh.numT + 2 * Mesh.numS + 9;
    end
    ...
    % DRUG INTERFACE
    % IVR / Lumen interface
    D_intf0a = (D.SA.*D.Dd_R.*sol(D_intf_RL,:)+(Geometry.SA.*D.Dd_L.*sol(D_intf_RL + 1,:)))./(Geometry.SA.*D.Dd_L.*D.phi_RL+D.SA.*D.Dd_R);
    D_intf0b = (D.SA.*D.Dd_R.*sol(D_intf_RL,:)+(Geometry.SA.*D.Dd_L.*sol(D_intf_RL + 1,:)))./(D.SA.*D.Dd_R./D.phi_RL+Geometry.SA.*D.Dd_L);
    % Lumen / epi interface
    ...
    ii = D_RIndices(1);
        d2soldx2(ii,:) = 2 * (sol(ii+1,:) - sol(ii,:)) ./ (dxR.^2);
        dsoldt(ii,:) = D.Dd_R .* d2soldx2(ii,:);

    ii = D_RIndices(2:end-1);
        d2soldx2(ii,:) = (sol(ii+1,:) - 2.*sol(ii,:) + sol(ii-1,:))./(dxR.^2);
        dsoldt(ii,:) = D.Dd_R .* d2soldx2(ii,:);

    ii = D_RIndices(end);
        d2soldx2(ii,:) = (D_intf0a - 2.*sol(ii,:) + sol(ii-1,:))./(dxR.^2);
        dsoldt(ii,:) = D.Dd_R .* d2soldx2(ii,:);

    ii = D_LIndices(1);
        d2soldx2(ii,:) = (sol(ii+1,:) - 2.*sol(ii,:) + D_intf0b)./(dxL.^2);
